...
function createElectricBlips()
    if Config.Electric.Blips.onlyShowNearestBlip then
        Citizen.CreateThread(function()
            local currentBlip = 0
            local currentBlipIndex = 0
            ...
            while true do
                ...
                for blipId, station in pairs(stations) do
                    local distanceToBlip = #(coords - station.center)

                    if distanceToBlip < closestBlipDistance then
                        closestBlipDistance = distanceToBlip
                        closestBlipCoords = station.center
                        closestBlipId = blipId
                    end
                end
                ...
                if currentBlipIndex ~= closestBlipId then
                    ...
                    if closestBlipCoords then
                        currentBlip = Utils.Blips.createBlipForCoords(closestBlipCoords.x,closestBlipCoords.y,closestBlipCoords.z,Config.Electric.Blips.blipId,Config.Electric.Blips.color,Utils.translate('blip_text_electric'),Config.Electric.Blips.scale,false)
                    end
        ...
        local text = Utils.translate('blip_text_electric')
        local stations = groupChargersByStation()
        for _, station in pairs(stations) do
            Utils.Blips.createBlipForCoords(station.center.x,station.center.y,station.center.z,Config.Electric.Blips.blipId,Config.Electric.Blips.color,text,Config.Electric.Blips.scale,false)
        end
    end
end
...
Citizen.CreateThread(function()
    ...
    if Config.Electric.Blips and Config.Electric.Blips.enabled then
        createElectricBlips()
    end
